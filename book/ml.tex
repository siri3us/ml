...
Выход рекуррентной сети по факту имеет вид
\begin{gather*}
...
\begin{itemize}
	...
	\item нам известны производная функции потерь $\partial \Loss / \partial \boldW$,$\partial \Loss / \partial \boldW$ 
\end{itemize}
Иными словами, функция потерь представляется как функция от выхода данного слоя, и параметров
$$
\Loss = \Loss(\boldy, \boldV, \boldW, \boldb),
$$
где параметры $\boldV, \boldW, \boldb$ оказывают влияние только через последующие слои. Чтобы включить сюда влияние через текущий слой, заметим, что это влияние осуществляется через выход $\boldy$ текущего слоя. Иными словами
\begin{gather*}
\frac{d\Loss}{d\boldW_{i,j}} = \frac{d\Loss(\boldy, \boldV, \boldW, \boldb)}{d\boldW_{i,j}} = \partder{\Loss}{\boldW_{i, j}} + \left(\partder{\boldy}{\boldW_{i,j}}\right)^T \partder{\Loss}{\boldy}
\end{gather*}
